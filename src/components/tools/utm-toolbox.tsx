      ...
      <div className="rounded-lg border border-slate-200 bg-slate-50 p-5">
        ...
        {mode === "utm-validator" ? (
          ...
          <div className="mt-5 overflow-hidden rounded-md border border-slate-200 bg-white">
            {decoded.map((item) => (
              <div key={item.key} className="grid grid-cols-[120px_1fr] border-b border-slate-100 px-4 py-3 text-sm last:border-b-0">
                <span className="font-medium text-slate-700">{item.label}</span>
                <span className="break-all text-slate-600">{item.value || "Not set"}</span>
              </div>
            ))}
          </div>
        ) : (
          <pre className="mt-5 min-h-40 whitespace-pre-wrap break-all rounded-md border border-slate-200 bg-white p-4 text-sm leading-6 text-slate-700">
