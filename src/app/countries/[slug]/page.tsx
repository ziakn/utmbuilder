...
export default async function CountryPage({ params }: Props) {
  ...
  return (
    <Section>
      <Container>
        <h1 className="max-w-3xl text-4xl font-bold text-slate-950">UTM campaign tracking for {country.name}</h1>
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {platforms.slice(0, 3).map((platform) => (
            <Link key={platform.slug} href={`/platforms/${platform.slug}`} className="rounded-lg border border-slate-200 p-5 text-sm font-semibold text-slate-700 hover:text-slate-950">
